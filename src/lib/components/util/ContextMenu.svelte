      ...
      {#each items as item}
        ...
        {:else if item.children && item.children.length > 0}
          <li class="ctx-item has-submenu">
            <button type="button" class="submenu-trigger" disabled={item.disabled}>{item.label}</button>
            <ul class="ctx-submenu">
              {#each item.children as child}
                ...
                {:else}
                  <li>
                    <button type="button" onclick={() => handleItemClick(child)} disabled={child.disabled}>
                      {child.label}
                    </button>
                  </li>
                {/if}
        ...
        {:else}
          <li>
            <button type="button" onclick={() => handleItemClick(item)} disabled={item.disabled}>{item.label}</button>
          </li>
        {/if}
      {/each}
    </ul>
